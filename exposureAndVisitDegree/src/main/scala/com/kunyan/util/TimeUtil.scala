package com.kunyan.util

import java.math.BigInteger
import java.text.SimpleDateFormat
import java.util.{Calendar, Date, Locale}

/**
  * Created by wangcao on 2016/08/13.
  * 时间转换的方法
  */
 object TimeUtil {

  /**
    * 将时间戳转换成yyyyMMddHH格式
    *
    * @param timeStamp 时间戳
    * @return 年月日时
    * @note rowNum: 6
    */
  def getTime(timeStamp: String): String = {

    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyyMMddHH")
    val bigInt: BigInteger = new BigInteger(timeStamp)
    val date: String = sdf.format(bigInt)

    date
  }

  /**
    * 将时间戳转换成yyyy-MM-ddHH格式
    *
    * @param timeStamp 时间戳
    * @return 年月日时
    * @note rowNum: 6
    */
  def getTimeByHour(timeStamp: String): String = {

    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd-HH")
    val bigInt: BigInteger = new BigInteger(timeStamp)
    val date: String = sdf.format(bigInt)

    date
  }

  /**
    * 将时间戳转换成yyyy-MM-dd格式
    *
    * @param timeStamp 时间戳
    * @return 年月日时
    * @note rowNum: 6
    */
  def getDay(timeStamp: String): String = {

    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd")
    val bigInt: BigInteger = new BigInteger(timeStamp)
    val date: String = sdf.format(bigInt)

    date
  }

  /**
    * 将时间戳转换成yyyy-MM-dd格式
    *
    * @param timeStamp 时间戳
    * @return 年月日时
    * @note rowNum: 6
    */
  def getHour(timeStamp: String): String = {

    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd-HH")
    val bigInt: BigInteger = new BigInteger(timeStamp)
    val date: String = sdf.format(bigInt)

    date
  }

  /**
    * 将时间戳转换成YYYY-MM格式
    *
    * @param timeStamp 时间戳
    * @return 年-月
    * @note rowNum: 6
    */
  def getMonth(timeStamp: String): String = {

    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM")
    val bigInt: BigInteger = new BigInteger(timeStamp)
    val date: String = sdf.format(bigInt)

    date
  }

  /**
    * 将"yyyy-MM-dd-HH"转换成时间戳（得到该小时的0分0秒）
    *
    * @param time 年月日时
    * @return 时间戳
    * @note rowNum: 6
    */
  def getTimeStamp(time: String): Long = {

    val loc = new Locale("en")
    val fm = new SimpleDateFormat("yyyy-MM-dd-HH", loc)
    val dt2 = fm.parse(time)

    dt2.getTime
  }

  /**
    * 将"yyyy-MM-dd"转换成时间戳（得到一天的0点0时0分）
    *
    * @param time 日期数据
    * @return 对应的时间戳
    * @note rowNum: 6
    */
  def getTimeStampDay(time: String): Long = {

    val loc = new Locale("en")
    val fm = new SimpleDateFormat("yyyy-MM-dd", loc)
    val dt2 = fm.parse(time)

    dt2.getTime
  }

  /**
    * 获取昨天的日期
    *
    * @return 昨天的日期
    * @note rowNum: 5
    */
  def getLastDay: String = {

    val date = new Date(new Date().getTime - 60L * 60 * 1000 * 24)
    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd")

    sdf.format(date)
  }

  /**
    * 获取上周一的日期
    *
    * @return 上周一的日期
    * @note rowNum: 5
    */
  def getLastWeek: String = {

    val date = new Date(new Date().getTime - 60L * 60 * 1000 * 24 * 7)
    val sdf: SimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd")

    sdf.format(date)
  }

}
